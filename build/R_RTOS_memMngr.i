# 1 "../source/R_RTOS/R_RTOS_memMngr.c"
...
# 1 "../source/R_RTOS/R_RTOS_memMngr.c"
# 9 "../source/R_RTOS/R_RTOS_memMngr.c"
...
# 553 "../include/R_RTOS_inc.h"
typedef uint8_t SyncEleID;
# 616 "../include/R_RTOS_inc.h"
...
# 841 "../include/R_RTOS_inc.h"
typedef union sysTickEleIDUnion
{
    TskID tskID;
    SysFktID sysFktID;
} SysTickEleID;
# 856 "../include/R_RTOS_inc.h"
...
{
    uint8_t tskReferences[(uint8_t)((((uint8_t)0x4u) >> 3) + 1)];
...
# 1415 "../include/R_RTOS_inc.h"
#define NR_OF_MEMPOOLS (uint8_t)0x7u
# 1424 "../include/R_RTOS_inc.h"
...
typedef enum svcCode
{
    ...
    SVC_SEM_SIGNAL,
    SVC_MNTR_INIT_MNTR,
    SVC_MNTR_REQ_READ_ACC,
    SVC_MNTR_REQ_WRITE_ACC,
    SVC_MNTR_REL_ACC,
    SVC_EVT_SEND,
    SVC_EVT_RECV,
    SVC_TMR_SET,
...
# 10 "../source/R_RTOS/R_RTOS_memMngr.c" 2
# 1 "../include/R_RTOS_BitMagic.h" 1
# 10 "../include/R_RTOS_BitMagic.h"
...
# 56 "../include/R_RTOS_BitMagic.h"
uint8_t bitM_bitPos(uint32_t bits);
# 11 "../source/R_RTOS/R_RTOS_memMngr.c" 2
...
# 28 "../source/R_RTOS/R_RTOS_memMngr.c"
static MemMngrHead * freeMemLst = (MemMngrHead *) ((void *)0x0u);
# 38 "../source/R_RTOS/R_RTOS_memMngr.c"
static MemMngrHead * objCache = (MemMngrHead *) ((void *)0x0u);
# 47 "../source/R_RTOS/R_RTOS_memMngr.c"
static MemMngrHead * objCacheEnd = (MemMngrHead *) ((void *)0x0u);
# 62 "../source/R_RTOS/R_RTOS_memMngr.c"
static uint8_t buddyOffset = (uint8_t) 0x0u;
# 71 "../source/R_RTOS/R_RTOS_memMngr.c"
static uint8_t cacheSize = (uint8_t) 0x0u;

static volatile uint32_t memPoolsInUse = 0x0u;
static MemMngrHead *memPools[(uint8_t)0x7u ];
...
static MemSize setBlckMemSize(
                               ...
                               MemSize newSize )
{
    if ( ( (BlckSize) ( newSize ) ) < ((BlckSize)0x100u) )
    {
        ((blckToSetSizeOf)->flagsForMemBlock) |= (MemFlags)((MemFlags)0x1000u);
    }
    else
    {
        ((blckToSetSizeOf)->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x1000u))));
    }

    blckToSetSizeOf->flagsForMemBlock =
            (MemFlags) ( blckToSetSizeOf->flagsForMemBlock
                    & (MemFlags) ( ~((MemFlags)0xFFCu) ) );
    blckToSetSizeOf->flagsForMemBlock =
            (MemFlags) ( blckToSetSizeOf->flagsForMemBlock
                    | (MemFlags) ( (MemFlags) ( newSize ) & ((MemFlags)0xFFCu) ) );
...
RetCode initMEM( void )
{
    ...
    buddyOffset = (uint8_t) ( (uint32_t) ( memoryHEAPBASE )
            & (uint32_t) ((BlckSize)(((BlckSize)0x100u) - (BlckSize)1u)) );

    MemIndex prvBlck = failMemIndex;
    MemIndex curBlck = (MemIndex) 0x0u;

    while ( memLstTempElem != (stdPtr) ((void *)0x0u) )
    {

        memLstTempElem = memoryHEAPBASE + curBlck;
        ...
        setBlckMemSize( memLstTempElem, ((BlckSize)0x100u) );

        memLstTempElem->additionalFlags = (MemFlags) 0x0u;

        ((memLstTempElem)->additionalFlags) |= (MemFlags)((MemFlags)0x1u);

        setPRVBlckMemSize( memLstTempElem, ((BlckSize)0x100u) );
        ...
        curBlck = (MemIndex) ( curBlck
                + (MemIndex) ( (MemIndex) ((BlckSize)0x100u)
                        / (MemIndex) ((BlckSize)0x8u) ) );
    ...
    ((memLstTempElem)->additionalFlags) |= (MemFlags)((MemFlags)0x2u);



    memPoolsInUse = (uint32_t) ( (uint32_t) 0x1u << (uint8_t)0x7u );

    memPoolsInUse = (uint32_t) ( ( memPoolsInUse | -memPoolsInUse ) );

    return ((RetCode)0x1u);
}
# 196 "../source/R_RTOS/R_RTOS_memMngr.c"
static void delMemBlck( MemMngrHead * const blckToDelete )
{
    ...
    {
        freeMemLst = memoryHEAPBASE + blckToDelete->nxtMemBlck;
    }
}
# 223 "../source/R_RTOS/R_RTOS_memMngr.c"
static MemMngrHead *getAdjacentPrevBlck( MemMngrHead * const curBlck )
{
    MemMngrHead * const adjPrvBlck = (MemMngrHead*) ( (uint32_t) curBlck
            - (uint32_t) (BlckSize)(( curBlck )->additionalFlags & ((MemFlags)0xFFCu))
                                                   );

    return ( (uint32_t) adjPrvBlck <= (uint32_t) ( &__HeapBase ) ) ?
            (MemMngrHead *) ((void *)0x0u) : adjPrvBlck;
}
# 240 "../source/R_RTOS/R_RTOS_memMngr.c"
static MemMngrHead *getAdjacentNxtBlck( MemMngrHead * const curBlck )
{
    MemMngrHead * const adjNxtBlck = (MemMngrHead*) ( (uint32_t) curBlck
            + (uint32_t) (MemSize)(( curBlck )->flagsForMemBlock & ((MemFlags)0xFFCu)) );

    return ( (uint32_t) adjNxtBlck >= (uint32_t) ( &__HeapLimit ) ) ?
            (MemMngrHead *) ((void *)0x0u) : adjNxtBlck;
}
# 259 "../source/R_RTOS/R_RTOS_memMngr.c"
...
static MemIndex getIndex( MemMngrHead * const memBlck )
{
    ...
    return (MemIndex) ( (MemIndex) ( (uint32_t) memBlck
            - (uint32_t) ( memoryHEAPBASE ) )
                        >> (MemIndex) (MemIndex)(0x3u) );
}
# 288 "../source/R_RTOS/R_RTOS_memMngr.c"
...
# 310 "../source/R_RTOS/R_RTOS_memMngr.c"
...
# 346 "../source/R_RTOS/R_RTOS_memMngr.c"
static uint8_t mergeMemBlck( MemMngrHead ** blckToMerge )
{
    BlckSize split = (MemSize)(( *blckToMerge )->flagsForMemBlock & ((MemFlags)0xFFCu));
    MemMngrHead* tmpBlck;


    if ( ((MemFlags)((MemFlags)(( *blckToMerge )->flagsForMemBlock >> (MemFlags)((MemFlags)0x1u)) & (MemFlags)0x1u)) )
    {
        MemMngrHead * nxtBlck = (MemMngrHead *) getAdjacentNxtBlck(
                (MemMngrHead*) ( *blckToMerge ) );
        if ( ( nxtBlck == (MemMngrHead *) ((void *)0x0u) ) || ( (MemFlags)(((MemFlags)((MemFlags)(nxtBlck->flagsForMemBlock >> (MemFlags)((MemFlags)0x0u)) & (MemFlags)0x1u)) | ((MemFlags)((MemFlags)(nxtBlck->additionalFlags >> (MemFlags)((MemFlags)0xFu)) & (MemFlags)0x1u)))
                          ) )
        {
            return (uint8_t) 0x0u;
        }
        ...
        {
            if ( (MemSize)((*blckToMerge)->flagsForMemBlock & ((MemFlags)0xFFCu)) == (MemSize)(nxtBlck->flagsForMemBlock & ((MemFlags)0xFFCu))
                                                )
            {

                ( *blckToMerge )->nxtMemBlck = nxtBlck->nxtMemBlck;
                if ( nxtBlck->nxtMemBlck != failMemIndex )
                {
                ...
                if ( tmpBlck != (MemMngrHead *) ((void *)0x0u) )
                {
    ...
    else
    {
        ...
        if ( ( prevBlck == (MemMngrHead *) ((void *)0x0u) ) || ( (MemFlags)(((MemFlags)((MemFlags)(prevBlck->flagsForMemBlock >> (MemFlags)((MemFlags)0x0u)) & (MemFlags)0x1u)) | ((MemFlags)((MemFlags)(prevBlck->additionalFlags >> (MemFlags)((MemFlags)0xFu)) & (MemFlags)0x1u)))
                           ) )
        {
            return (uint8_t) 0u;
        }
        ...
        {
            if ( (MemSize)((*blckToMerge)->flagsForMemBlock & ((MemFlags)0xFFCu)) == (MemSize)(prevBlck->flagsForMemBlock & ((MemFlags)0xFFCu))
                                                 )
            {
    ...
    if ( (uint32_t) ( *blckToMerge ) > (uint32_t) getBuddyBlck(
            (MemMngrHead*) ( *blckToMerge ) ) )
    {
        ((( *blckToMerge ))->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x2u))));
    }
    else
    {
        ((( *blckToMerge ))->flagsForMemBlock) |= (MemFlags)((MemFlags)0x2u);
    }
    return (uint8_t) 1u;
}
# 450 "../source/R_RTOS/R_RTOS_memMngr.c"
static void reMergeMemBlck( MemMngrHead ** blckToMerge )
{
    while ( ((MemFlags)((MemFlags)(( *blckToMerge )->flagsForMemBlock >> (MemFlags)((MemFlags)0xCu)) & (MemFlags)0x1u)) )
    {
        if ( !mergeMemBlck( blckToMerge ) )
            break;
    }
}
# 466 "../source/R_RTOS/R_RTOS_memMngr.c"
static void splitMemBlck( MemMngrHead ** const blckToSplit )
{
    const MemFlags split = (MemFlags) ( (MemSize)(( ( *blckToSplit )->flagsForMemBlock ) & ((MemFlags)0xFFCu))
    ...
    ((( *blckToSplit ))->flagsForMemBlock) |= (MemFlags)((MemFlags)0x2u);
}
# 507 "../source/R_RTOS/R_RTOS_memMngr.c"
static void unMergeUnevenBlcks( MemMngrHead * const blckToUnMerge )
{
    ...
    while ( (MemSize)(((MemSize)(blckToUnMerge->flagsForMemBlock & ((MemFlags)0xFFCu)) & (MemSize)((MemSize)(blckToUnMerge->flagsForMemBlock & ((MemFlags)0xFFCu))-(MemSize)1u)) ? 1u : 0u) )
    {
        smllrBlckSize = (MemSize)((MemSize)(( blckToUnMerge )->flagsForMemBlock & ((MemFlags)0xFFCu)) & (MemSize)(-(MemSize)(( blckToUnMerge )->flagsForMemBlock & ((MemFlags)0xFFCu))))
                                                     ;
        origBlckSize = (MemSize) ( (MemSize)(( blckToUnMerge )->flagsForMemBlock & ((MemFlags)0xFFCu))
        ...
        if ( (uint32_t) ( mrgdBlck ) > (uint32_t) getBuddyBlck(
                (MemMngrHead*) ( mrgdBlck ) ) )
        {
            ((( mrgdBlck ))->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x2u))));
        }
        else
        {
            ((( mrgdBlck ))->flagsForMemBlock) |= (MemFlags)((MemFlags)0x2u);
        }

        ((mrgdBlck)->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x1u))));
        setPRVBlckMemSize( mrgdBlck, ( origBlckSize ) );
        fitBlckBackIntoFreeLst( mrgdBlck );

        if ( smllrBlckSize < ((BlckSize)0x100u) )
            reMergeMemBlck( &mrgdBlck );

        setBlckMemSize( ( blckToUnMerge ), (MemSize) ( origBlckSize ) );
    }
}
# 589 "../source/R_RTOS/R_RTOS_memMngr.c"
static void mergeUnevenBlocks(
                               MemMngrHead* const basis,
                               MemMngrHead* const added )
{
    const MemSize newSize = (MemSize) (
    (MemSize)(basis->flagsForMemBlock & ((MemFlags)0xFFCu)) + (MemSize)(added->flagsForMemBlock & ((MemFlags)0xFFCu))
                                      );
    if ( getAdjacentNxtBlck( added ) != (MemMngrHead *) ((void *)0x0u) )
    {
    ...
    if ( (MemSize)(added->flagsForMemBlock & ((MemFlags)0xFFCu))

         > (MemSize)(basis->flagsForMemBlock & ((MemFlags)0xFFCu)) )
        ((basis)->additionalFlags) |= (MemFlags)((MemFlags)0x4000u);
...
static void shrinkCacheFIFO( void )
{
    ...
    {
        if ( objCacheEnd == (MemMngrHead*) ((void *)0x0u) )
        {
            BREAK();
        }
        ...
        if ( objCacheEnd->prvMemBlck == failMemIndex )
        {
            ...
            objCache = (MemMngrHead *) ((void *)0x0u);
            prvEndNode = (MemMngrHead *) ((void *)0x0u);
        }
        else
        {
            ...
            prvEndNode = &memoryHEAPBASE[objCacheEnd->prvMemBlck];
        }

        ((objCacheEnd)->additionalFlags) &= ((MemFlags)(~((MemFlags)((MemFlags)0x8000u))));
...
# 664 "../source/R_RTOS/R_RTOS_memMngr.c"
static void addBlckToCache( MemMngrHead * const blckToAdd )
{
    if ( ( ((MemFlags)((MemFlags)(blckToAdd->additionalFlags >> (MemFlags)((MemFlags)0xFu)) & (MemFlags)0x1u)) ) )
    {
    ...
    if ( (uint32_t) objCache == (uint32_t) ((void *)0x0u) )
    {
    ...
    ((blckToAdd)->additionalFlags) |= (MemFlags)((MemFlags)0x8000u);

    if ( ++cacheSize > ((uint8_t)0x10u) )
    {
        shrinkCacheFIFO();
    }
}
...
static MemMngrHead *findBestFitInCache( register MemSize requestedSlabSize )
{
    if ( (uint32_t) objCache != (uint32_t) ((void *)0x0u) )
    {
        ...
        if ( requestedSlabSize & (MemSize) ((BlckSize)(((BlckSize)0x8u) - (BlckSize)1u)) )
        {
            ...
            requestedSlabSize &= (MemSize) ( ~((BlckSize)(((BlckSize)0x8u) - (BlckSize)1u)) );
            requestedSlabSize = (MemSize) ( requestedSlabSize + ((BlckSize)0x8u) );
        }
        ...
        while ( ( (MemSize)(curBlckCche->flagsForMemBlock & ((MemFlags)0xFFCu))
                != requestedSlabSize ) )
        {
            if ( curBlckCche->nxtMemBlck == failMemIndex )
            {

                return (MemMngrHead *) ((void *)0x0u);
            }
        ...
        ((curBlckCche)->additionalFlags) &= ((MemFlags)(~((MemFlags)((MemFlags)0x8000u))));
...
# 760 "../source/R_RTOS/R_RTOS_memMngr.c"
        ...
        else
        {
            ...
            {
                objCache = (MemMngrHead*) ((void *)0x0u);
                objCacheEnd = (MemMngrHead*) ((void *)0x0u);
            }
            else
            {
...
{
    MemMngrHead * bestFitMemBlck = (MemMngrHead *) ((void *)0x0u);
    MemMngrHead * memLstTempElem = *strtToLookFor;
    MemSize curBestFitSize = (MemSize) 0xFFFFu;

    if ( memLstTempElem == (MemMngrHead *) ((void *)0x0u) )
        return (MemMngrHead *) ((void *)0x0u);
    ...
    do
    {
        ...
        if ( neededSize <= (MemSize)(memLstTempElem->flagsForMemBlock & ((MemFlags)0xFFCu))
                                                   )
        {

            if ( curBestFitSize > (MemSize)(memLstTempElem->flagsForMemBlock & ((MemFlags)0xFFCu))
                                                       )
            {
                curBestFitSize = (MemSize)(memLstTempElem->flagsForMemBlock & ((MemFlags)0xFFCu))
                                                          ;
                bestFitMemBlck = memLstTempElem;
            }
        }

        memLstTempElem = &memoryHEAPBASE[memLstTempElem->nxtMemBlck];

        if ( ( (uint32_t) memLstTempElem < (uint32_t) ( &__HeapBase ) ) || ( (uint32_t) memLstTempElem
                > (uint32_t) ( &__HeapLimit ) ) )
            break;
    } while ( memLstTempElem != (MemMngrHead *) ((void *)0x0u) );


    if ( bestFitMemBlck == (MemMngrHead *) ((void *)0x0u) )
        return (MemMngrHead *) ((void *)0x0u);


    while ( neededSize <= (MemSize) ( (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu))

                                      >> (MemSize) 0x1u ) )
    {
        splitMemBlck( &bestFitMemBlck );
        if ( (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu)) == ((BlckSize)0x8u) )
            break;
    }

    if ( ( (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu))

           > neededSize )
         && ( (MemSize) (
         (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu)) - neededSize )
              > ((BlckSize)0x8u) ) )
    {
        splitMemBlck( &bestFitMemBlck );

        while ( ( (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu))
                  ...
                  < neededSize ) )
        {
            ...
            MemSize addedNeededSize = (MemSize) ( neededSize
                    - (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu)) );


            while ( addedNeededSize <= (MemSize) ( (MemSize)(additionalBlck->flagsForMemBlock & ((MemFlags)0xFFCu))

                                                   >> (MemSize) 0x1u ) )
            {
                if ( (MemSize)(additionalBlck->flagsForMemBlock & ((MemFlags)0xFFCu)) == ((BlckSize)0x8u) )
                    break;
                splitMemBlck( &additionalBlck );
            }


            if ( ( neededSize > ( (MemSize)(bestFitMemBlck->flagsForMemBlock & ((MemFlags)0xFFCu))

                                  + ( (MemSize)(additionalBlck->flagsForMemBlock & ((MemFlags)0xFFCu))

                                      >> (MemSize) 0x1u ) ) )
                 && ( ( (MemSize)(additionalBlck->flagsForMemBlock & ((MemFlags)0xFFCu)) >> (MemSize) 0x1u ) > ((BlckSize)0x8u) ) )
            {
                splitMemBlck( &additionalBlck );
...
{
    const MemSize sizeToCpy = (MemSize)(src->flagsForMemBlock & ((MemFlags)0xFFCu));
    MemSize cntr = sizeof(uint32_t);

    if ( ( dest == (MemMngrHead *) ((void *)0x0u) ) || ( src == (MemMngrHead *) ((void *)0x0u) ) )
        return;
...
void * rMalloc( const MemSize desiredSize )
{
    ...
    neededSize = ( neededSize < ((BlckSize)0x8u) ) ? ((BlckSize)0x8u) : neededSize;
    MemMngrHead *tmp = findBestFitInCache( neededSize );
    if ( (uint32_t) tmp == (uint32_t) ((void *)0x0u) )
    {
        tmp = findBestFitFree( &freeMemLst, neededSize );
        while ( ( (uint32_t) tmp == (uint32_t) ((void *)0x0u) ) && ( (uint32_t) objCache
                != (uint32_t) ((void *)0x0u) ) )
        {
            shrinkCacheFIFO();
            tmp = findBestFitFree( &freeMemLst, neededSize );
    ...
    ((tmp)->flagsForMemBlock) |= (MemFlags)((MemFlags)0x1u);

    return ((void *) ((uint32_t) tmp + (uint32_t) sizeof(MemBlckHead)));
}
...
{
    if ( ptrToExistingMem == ((void *)0x0u) )
    {
        return rMalloc( desiredSize );
    }
    else
    {
        ...
        neededSize =
                ( neededSize < ((BlckSize)0x8u) ) ? ((BlckSize)0x8u) : neededSize;

        MemMngrHead * tmp = ((MemMngrHead *)( (uint32_t) ptrToExistingMem - (uint32_t) sizeof(MemBlckHead)));
        MemMngrHead * tmpNxt;
        while ( (MemSize)(tmp->flagsForMemBlock & ((MemFlags)0xFFCu)) < neededSize )
        {
            tmpNxt = (MemMngrHead *) getAdjacentNxtBlck( (MemMngrHead *) tmp );

            if ( (MemFlags)(((MemFlags)((MemFlags)(tmpNxt->flagsForMemBlock >> (MemFlags)((MemFlags)0x0u)) & (MemFlags)0x1u)) | ((MemFlags)((MemFlags)(tmpNxt->additionalFlags >> (MemFlags)((MemFlags)0xFu)) & (MemFlags)0x1u))) )
                break;
            ...
            setBlckMemSize(
                    tmp,
                    (MemSize) ( (MemSize)(tmp->flagsForMemBlock & ((MemFlags)0xFFCu))
                            + (MemSize)(tmpNxt->flagsForMemBlock & ((MemFlags)0xFFCu)) ) );
            setPRVBlckMemSize(
                    tmpNxt, (MemSize)(tmp->flagsForMemBlock & ((MemFlags)0xFFCu)) );
        }
        if ( (MemSize)(tmp->flagsForMemBlock & ((MemFlags)0xFFCu)) < neededSize )
        {
            MemMngrHead * tmp2 = rMalloc( desiredSize );
...
{
    if ( (uint32_t) pToBeFreed == (uint32_t) ((void *)0x0u) )
        BREAK();

    MemMngrHead * const tmp = (MemMngrHead *) ((MemMngrHead *)( (uint32_t) pToBeFreed - (uint32_t) sizeof(MemBlckHead)))
                        ;

    if ( !( ((MemFlags)((MemFlags)(tmp->flagsForMemBlock >> (MemFlags)((MemFlags)0x0u)) & (MemFlags)0x1u)) ) )
        BREAK();

    ((tmp)->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x1u))));
    addBlckToCache( tmp );
}
...
void rFullyFree( MemMngrHead * blckToFullyFree )
{
    ((blckToFullyFree)->flagsForMemBlock) &= ((MemFlags)(~((MemFlags)((MemFlags)0x1u))));

    if ( (MemSize)(((MemSize)(blckToFullyFree->flagsForMemBlock & ((MemFlags)0xFFCu)) & (MemSize)((MemSize)(blckToFullyFree->flagsForMemBlock & ((MemFlags)0xFFCu))-(MemSize)1u)) ? 1u : 0u) )
    {
    ...
    if ( ((MemFlags)((MemFlags)(blckToFullyFree->flagsForMemBlock >> (MemFlags)((MemFlags)0xCu)) & (MemFlags)0x1u)) )
    {
                               ...
                               MemPoolID * const memPoolID )
{
    ...
    *memPoolID = bitM_bitPos(
    ...
    if ( (uint32_t) curMemBlock == (uint32_t) ((void *)0x0u) )
        return ((RetCode)0x0u);

    curMemBlock = ((MemMngrHead *)( (uint32_t) curMemBlock - (uint32_t) sizeof(MemBlckHead)));
    ...
    while ( elementCntr-- )
    {
        ...
        if ( (uint32_t) ( nxtMemBlock ) != (uint32_t) ((void *)0x0u) )
        {
            nxtMemBlock = ((MemMngrHead *)( (uint32_t) nxtMemBlock - (uint32_t) sizeof(MemBlckHead)));
...
RetCode memMngr_MemPoolMalloc( void ** ptrToMem, const MemPoolID memPoolID )
{
    ...
    if ( (uint32_t) memBlck != (uint32_t) ((void *)0x0u) )
    {
        ...
        {
            memPools[memPoolID] = (MemMngrHead *) ((void *)0x0u);
        }
        *ptrToMem = ((void *) ((uint32_t) memBlck + (uint32_t) sizeof(MemBlckHead)));
        return ((RetCode)0x1u);
    }
    BREAK();
    return ((RetCode)0x0u);
...
RetCode memMngr_MemPoolFree( void * ptrToMem, const MemPoolID memPoolID )
{
    if ( (uint32_t) ptrToMem == (uint32_t) ((void *)0x0u) )
        return ((RetCode)0x0u);

    MemMngrHead *memBlck = ((MemMngrHead *)( (uint32_t) ptrToMem - (uint32_t) sizeof(MemBlckHead)));
    if ( (uint32_t) memPools[memPoolID] != (uint32_t) ((void *)0x0u) )
    {
