# 1 "../source/R_RTOS/R_RTOS_monitor.c"
...
# 1 "../source/R_RTOS/R_RTOS_monitor.c"
# 9 "../source/R_RTOS/R_RTOS_monitor.c"
...
# 10 "../source/R_RTOS/R_RTOS_monitor.c" 2
# 1 "../include/R_RTOS_memMngr.h" 1
# 11 "../source/R_RTOS/R_RTOS_monitor.c" 2
...
# 12 "../source/R_RTOS/R_RTOS_monitor.c" 2

extern TskTCB tsk_AR[((uint8_t)0x4u)];

static MemPoolID memPoolID_MNTR;

static Mntr ar_Mntr[(uint8_t)0x2u ];

static RetCode mntr_appendToWaitrList(
                                       PMntr const mntr,
                                       PTskTCB const waitrTsk )
{
    if ( mntr->mntrWaitrsQTskIDEnd == ((TskID)0x0u) )
    {

        waitrTsk->prvTsk = ((TskID)0x0u);
        waitrTsk->nxtTsk = ((TskID)0x0u);
    ...
    else
    {
        ...
        mntr->mntrWaitrsQTskIDEnd = waitrTsk->tskID;
        if ( mntr->mntrWaitrsQTskIDStrt == ((TskID)0x0u) )
        {
...
static RetCode mntr_getNextWaitr( PMntr const mntr )
{
    ...
    while ( mntr->mntrWaitrsQTskIDStrt != ((TskID)0x0u) )
    {
    ...
    if(mntr->mntrWaitrsQTskIDStrt == ((TskID)0x0u))
        mntr->mntrWaitrsQTskIDEnd = ((TskID)0x0u);
...
RetCode mntr_INIT( void )
{
    uint8_t mntrCntr = (uint8_t)0x2u;
    while(mntrCntr--)
    {
        mntr_InitMntr(mntrCntr, ((void *)0x0u));
    }
    return memMngr_CreateMemPool(
            sizeof(SyncEle), (uint8_t)0x4u, &memPoolID_MNTR );
}
...

{
    ...
    mntr->mntrWaitrsQTskIDStrt = ((TskID)0x0u);
    mntr->mntrWaitrsQTskIDEnd = ((TskID)0x0u);
    return ((RetCode)0x1u);
}
...
{
    PMntr mntr = &ar_Mntr[mntrNr];
# 136 "../source/R_RTOS/R_RTOS_monitor.c"
    return ((RetCode)0x1u);
}
...
RetCode mntr_ReqstReadAccssMntr( const MntrNr mntrNr, PTskTCB const tsk )
{
    ...
    else if ( mntr->curState & MNTR_FREE )
    {
    ...
    else if ( mntr->curState & MNTR_WRITE )
    {



        PSyncEle mntrWaitrRead;
        memMngr_MemPoolMalloc( &mntrWaitrRead, memPoolID_MNTR );
        if ( (uint32_t) mntrWaitrRead != (uint32_t) ((void *)0x0u) )
        {
            ...
            if ( tsk_SetInactive( tsk, TSK_STATE_WAITING_MNTR ) == ((RetCode)0x1u) )
            {
...
RetCode mntr_ReqstWriteAccssMntr( const MntrNr mntrNr, PTskTCB const tsk )
{
    ...
    else if ( mntr->curState == MNTR_FREE_NOOP )
    {
    ...
    else
    {
        ...
        PSyncEle mntrWaitrWrite;
        memMngr_MemPoolMalloc( &mntrWaitrWrite, memPoolID_MNTR );
        if ( (uint32_t) mntrWaitrWrite != (uint32_t) ((void *)0x0u) )
        {
