# 1 "../source/R_RTOS/R_RTOS_events.c"
...
# 1 "../source/R_RTOS/R_RTOS_events.c"
# 9 "../source/R_RTOS/R_RTOS_events.c"
...
# 11 "../source/R_RTOS/R_RTOS_events.c" 2
...
# 12 "../source/R_RTOS/R_RTOS_events.c" 2
...
# 13 "../source/R_RTOS/R_RTOS_events.c" 2
...
# 14 "../source/R_RTOS/R_RTOS_events.c" 2

extern void os_SCHEDULE( void );
extern TskTCB tsk_AR[((uint8_t)0x4u)];

static Evt gEvts[(uint8_t)0x4u ];
...
void evt_INIT( void )
{
    register uint8_t cntr = (uint8_t)0x4u;
    register uint8_t evtQCntr;
    while ( cntr-- )
    {
        gEvts[cntr].evtQ_Slots = (EVTQSlots) 0x0u;
        evtQCntr = (uint8_t)0x4u;
        while ( evtQCntr-- )
        {
            gEvts[cntr].evtObjs[evtQCntr] = ((TskID)0x0u);
        }
    }
    memMngr_CreateMemPool( sizeof(SyncEle), (uint8_t)0x4u, &memPoolID_EVT );
}
                         ...
                         const SysTicks maxWaitTime )
{
    ...
    if ( (uint32_t) evtSync != (uint32_t) ((void *)0x0u) )
    {
...
RetCode evt_SendEvt( const EvtNr evtNr )
{
    ...
    while ( evtQSlotsHelper )
    {
        ...
        evt->evtObjs[evtNrHelper] = ((TskID)0x0u);
        if(tsk->tskID == ((TskID)0x0u))
        {
            BREAK();
        }

        if ( ( (uint32_t) tsk->tskSync != (uint32_t) ((void *)0x0u) ) && !( tsk->tskSync->SyncEleHandle.EvtSyncEle.evtMsk &=
                ~( 0x1u << evtNr ) ) )
        {
