 ...
 *  Since a Binary Semaphore only depends on singaling, it can be signaled from anywhere, whereas a mutex can only be released by the task that occupies it.
 */
                     ...
                     const SysTicks maxSysTicksToWait )
{
    ...
    if ( pMtx->mtxOccTskID != TSK_ID_NO_TSK )
    {
        ...
        return sem_InsertTskSemQ( pMtx, tsk );    // insert the new semaphore according to its task's priority
    }
    else
    {
        ...
        // If the semaphore is free, there is also no waiting queue; to make sure, set the Q start to TSK_ID_NO_TSK; COULD BE OPTIMIZED OUT
